...
void resistenzaMain(){
    ...
    scanf(" %c", &tipo);                                       
    float *array = malloc(num * sizeof(float));        //consigliano l'Allocazione dinamica della memoria per l'array e di evitare fflush(stdin);
    if (array == NULL) {                              //errorExit
    ...
    if (risultato == -1) {printf("Tipo di connessione non valido.\n");}
                                                ...
                                                // Liberare la memoria allocata per l'array
    free(array);
}
/*-----------------------------------leggi di ohm-------------------------------------------------------------------------------*/
